...
if (isset($_GET['q'])) {
    $datos = array();
    $nombre = $_GET['q'];
    $cliente = mysqli_query($conexion, "SELECT * FROM cliente WHERE nombre LIKE '%$nombre%' LIMIT 10");
    ...
    $datos = array();
    $nombre = $_GET['pro'];
    $producto = mysqli_query($conexion, "SELECT * FROM producto WHERE codproducto LIKE '%" . $nombre . "%' OR descripcion LIKE '%" . $nombre . "%' LIMIT 10");
    ...
    if ($insertar) {
        ...
        while ($row = mysqli_fetch_assoc($consultaDetalle)) {
            ...
            $total = $row['total'];
            $insertarDet = mysqli_query($conexion, "INSERT INTO detalle_venta (id_producto, id_venta, cantidad, precio, total) VALUES ($id_producto, $ultimoId, $cantidad, '$precio', '$total')");
